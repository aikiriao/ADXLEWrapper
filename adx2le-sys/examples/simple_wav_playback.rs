...
fn main() {
    unsafe {
        ...
        // ボイスプール作成
        let mut voicepool_config = criAtomExVoicePool_GetDefaultConfigForWaveVoicePool();
        voicepool_config.player_config.streaming_flag = CRI_TRUE as i32;
        let voicepool = criAtomExVoicePool_AllocateWaveVoicePool(&voicepool_config, null_mut(), 0);

        // プレーヤ作成
        let mut player_config = criAtomExPlayer_GetDefaultConfig();
        player_config.max_path = 255;
        let player = criAtomExPlayer_Create(&player_config, null_mut(), 0);

        // wavファイルセット
        let path = CString::new("d.wav").unwrap();
        criAtomExPlayer_SetFile(player, null_mut(), path.as_ptr());
